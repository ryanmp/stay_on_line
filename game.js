...
function instantiate_game(in_seed, level_params, level_colors){
    ...
    // auto set some of the color params
    if (level_colors.bg.l > .5){
        level_colors.overlay.l = 0;
    } else {
        level_colors.overlay.l = 1;
    }
    ...
    var colorChanges = function () {
        ...
        var bg = level_colors.bg;
        bg.h = ((Math.sin(frame*bg.h_v)+1)/2)*(bg.h_max-bg.h_min)+bg.h_min;

        colors[0] = hslToRgb(bg.h,bg.s,bg.l,1);
        ...
        */
        ...
        var p = level_colors.player;
        var e = level_colors.enemy;

        //p.h =  Math.sin((frame*p.h_v)%Math.PI/2);
        p.h = ((Math.sin(frame*p.h_v)+1)/2)*(p.h_max-p.h_min)+p.h_min;
        e.h = (p.h + 0.5)%1;

        //p.s = Math.sin((frame*p.s_v)%Math.PI/2)*.0 + .3;
        //e.s = p.s;

        colors[1] = hslToRgb(p.h,p.s,p.l,1.0);
        colors[2] = hslToRgb(e.h,e.s,e.l,0.9);
    };
    ...
    var draw = function () {
        ...
        ctx.fillStyle = toRGB(hslToRgb(ol.h,ol.s,ol.l,.1*(1-abs_h*1.5/h))); ctx.fill();

        //player
        ctx.beginPath();
        ctx.arc(player.p.x, player.p.y,player.r, 0,2*Math.PI);

        console.log(hslToRgb(level_colors.player));

        ctx.fillStyle = toRGB(hslToRgb(level_colors.player));
        ...
        ctx.fillStyle = toRGB(hslToRgb(ol.h,ol.s,ol.l,.2));
    ...
    setInterval(function () {
        ...
        if (!dead && !paused){
            ...
            if (frame%(60*7)==0 && current_level < level_params.length-1)// every 8 seconds, advance levels
            { current_level += 1; }
            ...
            if (dead_ani_frame > 60){
                if (moving_up || moving_down){
                    ...
                    // reset everything!
                    Math.seed = in_seed; // hm.... i will probably need to figure out a way to reseed my random number gen as well (this doesn't work)
                    dead_ani_frame = 0;
                    dead = false;
                    current_level = 0;
                    enemies = [];
